      ...
      <v-app key="2" v-if="true" class="dashboard statusbackground">
        ...
        <div class="wrap">
          ...
          <div class="col col6">
            ...
            <div
              class="box box9"
              v-if="currentGroupIncludes('Awaiting Registration')"
            >
              <p class="big">Welcome.</p>
              <p class="small">
                Registration is now open. Your unique discord check-in code is
                <span id="code">{{ this.code }}</span
                >. <b>Please do not share this with others!</b> Click
                <a href="https://discord.gg/6HvMQAnMgb" id="disc-join">HERE</a>
                to join the server.
              </p>
            </div>
            <div
              class="box box9"
              v-if="currentGroupIncludes('submission not open')"
            >
              <p class="big">Welcome.</p>
              <p class="small">The hackathon has now started. Good Luck!</p>
            </div>
            <div
              class="box box9"
              v-if="currentGroupIncludes('missed submission deadline')"
            >
              <p class="big">Sadge.</p>
              <p class="small">
                Submissions are now closed. We would love to see you again next
                year!
              </p>
            </div>
            <div class="box box9" v-if="currentGroupIncludes('Registered')">
              <p class="big">Welcome.</p>
              <p class="small">
                Thank you for registering. We are looking forward to seeing you
                at the hackathon.
              </p>
            </div>
            <div class="box box9" v-if="currentGroupIncludes('No RSVP')">
              <p class="big">Welcome</p>
              <p class="small">
                The registration period is now over. RSVPs and applications are
                no longer being accepted. We would love to see you next year!
              </p>
            </div>
            <div class="box box9" v-if="currentGroupIncludes('Awaiting RSVP')">
              <p class="big rsvp" style="font-size: 5vw">
                Will you be attending DH8?
              </p>
              <a
                @click="
                  () => {
                ...
                "
              >
                <div class="box box3 rsvp-btn">Yes</div>
              </a>
              <a
                @click="
                  () => {
                ...
                "
              >
                <div class="box box3 rsvp-btn">No</div>
              </a>
            </div>
            ...
            <div
              class="box box9 boxclip"
              v-if="currentGroupIncludes('not submitted')"
            >
              <template>
            ...
            <div
              class="box box9"
              v-if="currentGroupIncludes('Project Submitted')"
            >
              <template>
                <div class="afterSubmit">
                  <div style="float: left">
  ...
  data(): StatusModel {
    return {
      ...
      state_data: {
        ...
        accepted: {
          message: "Congratulations, you've been accepted!",
          emoji: "🥳",
          groups: ["Awaiting RSVP"],
        },
        coming: {
          message: "Congratulations, you're coming to DH8.",
          emoji: "🎉",
          groups: ["RSVPd", "coming"],
        },
        "open registration": {
          ...
          emoji: "🔓",
          groups: ["Awaiting Registration", "coming"],
        },
        "closed rsvp": {
        ...
        waiting: {
          ...
          emoji: "⏳",
          groups: ["Registered", "coming", "showStartTime"],
        },
        "checked in": {
          message: "Welcome to DH8.",
          emoji: "✅",
          groups: ["At hackathon", "showDeadline", "submission not open"],
        },
        submitting: {
          message: "Good Luck!",
          emoji: "💻",
          groups: ["At hackathon", "not submitted"],
        },
        "project submitted": {
          message: "Your project has been submitted.",
          emoji: "🔥",
          groups: ["At hackathon", "Project Submitted"],
        },
        "group assigned": {
          message: "Please join your designated voice channel.",
          emoji: "💯",
          groups: ["At hackathon", "Project Submitted"],
        },
        "submissions closed": {
          ...
          emoji: "😢",
          groups: ["Cannot Submit", "missed submission deadline"],
        },
        "not coming": {
          message: "Not coming to DH8.",
          emoji: "😭",
          groups: ["RSVPd", "Not coming", "Cannot Submit"],
        },
        rejected: {
          message: "Sorry, we couldn't offer you a spot this year.",
          emoji: "🙁",
          groups: ["rejected", "Not coming", "Cannot Submit"],
        },
        "applications closed": {
          message: "Applications are now closed.",
          emoji: "🔒",
    ...
    async updateStep(email: string) {
      ...
      const x = await db
        ...
        .onSnapshot((snap) => {
          ...
          if (snap.data()!._.status === "submitted")
      ...
      if (!this.projectSubmitted) {
        ...
        if (signupRequest.data.projectSubmitted) {
          ...
          this.groupNumber =
            signupRequest.data.tableNumber !== -1
              ? signupRequest.data.tableNumber
              : "Pending";
        }
      }
      ...
      db.collection(this.hackathon)
        ...
        .onSnapshot((snap) => {
          if (snap.exists) {
            ...
            if (data!._.status) {
              if (
                data!._.status === "in progress" ||
                data!._.status === "submitted"
              ) {
                this.current_state = data!._.status;
              }
            }
            if (data!._.decision) {
              if (data!._.decision === "rejected") {
                this.current_state = "rejected";
                return;
              }
              if (data!._.decision.substring(0, 5) === "round") {
            ...
            if (
              (data!._.RSVP.coming == null || !data!._.RSVP.coming) &&
              this.rsvpClosed
            ) {
