  ...
  >
    ...
    <div class="container-status100">
      <div class="wrap-apply100">
        ...
        >
          ...
          <h1 v-else class="text-xs-left">
            You've submitted your application. 😁
          </h1>
          <p v-if="!submitted" class="text-xs-left">
            Please fill out this application form to the best of your abilities.
            This form will autosave, you can come back to submit it any time
            before the deadline.
          </p>
          <p v-if="!submitted" class="text-xs-left">
            No programming experience? That's okay! We're just looking for well
            thought out answers. The more thought out your answers, the greater
            your chance of getting accepted. Only the questions under
            "Application Questions" will be judged. You'll get an email when
            we've made a decision.
          </p>
          <p v-if="!submitted" class="text-xs-left">
            Are you a high school student? You're eligible to attend - as long
            as you're 18 or older on the day of the event, Jan 26, 2019.
          </p>
          <p v-if="!submitted" class="text-xs-left">
            Round one has now concluded. Applications will now be considered for
            round two, which closes on Nov 15.
          </p>
          <p v-else class="text-xs-left">
            Your application has been recieved by us, sit back and relax while
            our team reviews it. You'll be notified when a decision is made, in
            the meantime you can check the status page to check the progress of
            your application. If you wish to make any changes you can email
            relations@deltahacks.com.
          </p>
          <br />
          <br />
          ...
          <v-text-field
            ...
            id="date"
            :disabled="submitted"
            v-model="application.personal.birthday"
            mask="date"
          ...
          <label for="date" style="float:left">
            <strong
              :class="enableGenderSelect && submitted ? 'we_messed_up' : ''"
            >
              What's your gender? *
            </strong>
          </label>
          ...
          <v-select
            name="gender"
            id="gender"
            :disabled="!enableGenderSelect && submitted"
            @change="genderChange"
            v-model="application.personal.gender"
            :items="['M', 'F', 'Other/Prefer not to say']"
            v-validate="{ required: true }"
          ...
          <v-select
            ...
            name="race"
            :disabled="submitted"
            v-model="application.personal.race"
            @change="formChange"
            v-validate="{ required: true }"
            :error-messages="errors.first('race:required')"
            data-vv-delay="1000"
          ></v-select>
          ...
          <v-combobox
            ...
            v-model="application.academics.university"
            v-validate="{ required: true }"
            :error-messages="errors.first('university:required')"
            data-vv-delay="1000"
          >
          ...
          <v-combobox
            ...
            :items="majors"
            @change="formChange"
            v-model="application.academics.major"
            v-validate="{ required: true }"
            :error-messages="errors.first('major')"
            data-vv-delay="1000"
          >
            <template slot="no-data">
              <v-list-tile>
          ...
          <v-select
            ...
            :items="degrees"
            @change="formChange"
            v-model="application.academics.degree"
            v-validate="{ required: true }"
            :error-messages="errors.first('degree')"
            data-vv-delay="1000"
          ></v-select>
          ...
          <v-select
            ...
            :disabled="submitted"
            @change="formChange"
            v-model="application.academics.year"
            :items="items"
            v-validate="{ required: true }"
            :error-messages="errors.first('year:required')"
            data-vv-delay="1000"
          ></v-select>
          ...
          <v-select
            ...
            name="shirt size"
            id="shirt size"
            :error-messages="errors.first('shirt size:required')"
            data-vv-delay="1000"
          ></v-select>
          ...
          <v-combobox
            ...
            name="diet"
            id="diet"
            :error-messages="errors.first('diet:required')"
            data-vv-delay="1000"
          >
            <template slot="no-data">
              <v-list-tile>
          ...
          <br />
          <v-select
            v-model="application.hackathons"
            @change="formChange"
            :disabled="submitted"
            :items="hackathons"
            v-validate="{ required: true }"
            name="hackathons"
            id="hackathons"
            :error-messages="errors.first('hackathons:required')"
            data-vv-delay="1000"
          ></v-select>
          <v-flex xs12>
          ...
          <v-select
            ...
            name="discover"
            id="diet"
            :error-messages="errors.first('discover:required')"
            data-vv-delay="1000"
          ></v-select>
          ...
          <v-combobox
            ...
            @change="formChange"
            v-validate="{ required: true }"
            :error-messages="errors.first('location:required')"
            data-vv-delay="1000"
          >
            <template slot="no-data">
              <v-list-tile>
          ...
          <v-text-field
            ...
            v-validate="{
              required: true,
              max: 11,
              is_not: application.emergency.phone,
            }"
            :error-messages="errors.first('phone:required')"
          ></v-text-field>
          ...
          <v-text-field
            ...
            v-validate="{
              required: true,
              max: 11,
              is_not: application.contact.phone,
            }"
            :error-messages="errors.first('emergency phone')"
          ></v-text-field>
          ...
          <v-text-field
            ...
            :disabled="submitted"
            autocomplete="off"
            v-model="application.emergency.name"
            v-validate="{ required: true, max: 100 }"
            :error-messages="errors.first('emergency name')"
            data-vv-delay="1000"
          ></v-text-field>
          ...
          <v-select
            ...
            v-model="application.emergency.relation"
            v-validate="{
              required: true,
              max: 11,
              is_not: application.contact.phone,
            }"
            :error-messages="errors.first('emergency relationship')"
          ></v-select>
          <div class="section divider"></div>
          <p>
            Note : Only information below this line will be considered during
            the reviewing process. The above information is for statistics,
            demographics, and logistical purposes only.
          </p>
          <div class="section divider"></div>
          ...
          <v-text-field
            ...
            single-line
            data-vv-delay="4000"
            v-model="application.profiles.github"
            prepend-icon="fab fa-github"
            v-validate="{ max: 150, url: true }"
            :error-messages="errors.first('github')"
          ></v-text-field>
          <v-text-field
            name="linkedin"
            :disabled="submitted"
            ...
            single-line
            data-vv-delay="4000"
            v-model="application.profiles.linkedin"
            prepend-icon="fab fa-linkedin"
            v-validate="{ max: 150, url: true }"
            :error-messages="errors.first('linkedin')"
          ></v-text-field>
          <v-text-field
            name="website"
            :disabled="submitted"
            ...
            single-line
            data-vv-delay="4000"
            v-model="application.profiles.website"
            prepend-icon="fas fa-link"
            v-validate="{ max: 150, url: true }"
            :error-messages="errors.first('website')"
          ></v-text-field>
          <v-text-field
            name="devpost"
            :disabled="submitted"
            ...
            single-line
            data-vv-delay="4000"
            v-model="application.profiles.devpost"
            prepend-icon="fas fa-link"
            v-validate="{ max: 150, url: true }"
            :error-messages="errors.first('devpost')"
          ></v-text-field>
          <br />
          <div id="filePondContainer">
            <file-pond
              @addfile="submitFileInfoOnDrop"
              v-if="!submitted"
              name="test"
              ref="pond"
              label-idle="Drop resume here..."
            ...
            <v-chip
              class="no border"
              style="float:left;overflow: wrap"
              v-if="haveFile"
              outline
              small
              color="gray"
            >
              ...
              <a target="_blank" :href="application.documents.download_link">
                {{ application.documents.filename }}
              </a>
              "
            </v-chip>
          </div>
          <div class="section divider"></div>
          <h2 style="float:left; padding-left:26px;">
            {{ subsectionLabels[4] }}
          </h2>
          <br />
          <br />
            ...
            <v-layout row wrap>
              <v-flex xs12>
                <p class="text-lg-left">
                  Tell us about a project you worked on/ thing you made/
                  internship you did/ course you took that you are really
                ...
                <v-textarea
                  ...
                  autocomplete="nope"
                  name="project"
                  placeholder="Tell us about a project you've worked on recently..."
                  v-model="application.responses.q1"
                  auto-grow
                  v-validate="{ required: true, max: 500 }"
                  :error-messages="errors.first('project')"
                  counter="500"
                ></v-textarea>
                <v-progress-linear
                  v-if="custom"
              ...
              <v-flex xs12>
                <p class="text-lg-left">
                  Why do you want to come to Deltahacks V, and what is one thing
                  that you are passionate to bring to this year's hackathon?*
                </p>
                <v-textarea
                  box
                  :disabled="submitted"
                  autocomplete="nope"
                  name="reason"
                  placeholder="Why do you want to come to Deltahacks V..."
                  v-model="application.responses.q2"
                  auto-grow
                  v-validate="{ required: true, max: 500 }"
                  :error-messages="errors.first('reason')"
                  counter="500"
                ></v-textarea>
                <v-progress-linear
                  v-if="custom"
              ...
              <v-flex xs12>
                <p class="text-lg-left">
                  If you could teleport to anywhere in the world right now,
                ...
                <v-textarea
                  ...
                  autocomplete="nope"
                  v-model="application.responses.q3"
                  auto-grow
                  v-validate="{ required: true, max: 500 }"
                  :error-messages="errors.first('creative')"
                  counter="500"
                />
                <v-progress-linear
                  v-if="custom"
              ...
              <v-flex xs12>
                ...
                <v-text-field
                  ...
                  v-model="application.responses.q4"
                  auto-grow
                />
              </v-flex>
              <div class="section divider"></div>
          ...
          <div class="small font">
            <v-checkbox
              ...
              id="mlh"
              v-model="checkbox"
              :error-messages="checkError"
            >
              <span style="font-size:0.8em;" class="terms" slot="label">
                {{ MLH }}
                <a @click.stop href="https://mlh.io/privacy">
                  MLH Privacy Policy
                </a>
                . I further agree to the
                <a @click.stop :href="contest_terms">
            ...
            >
              <span style="font-size:0.8em;" slot="label">
          ...
          <div class="mx-auto gg">
            <v-dialog v-model="confirmGender" persistent max-width="400">
              <v-card>
                <v-card-title class="headline">
                  Are you sure you want to update your application with this
                  answer?
                </v-card-title>
                <v-card-text>
                  You cannot edit your answer once you've done this.
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn
                    color="red darken-1"
                    flat
                    @click.native="confirmGender = false"
                  >
                    No
                  </v-btn>
                  <v-btn color="green darken-1" flat @click="redirectToStatus">
                    Yes
                  </v-btn>
                </v-card-actions>
              </v-card>
            ...
            <v-dialog v-model="confirm" persistent max-width="400">
              <v-btn slot="activator" :disabled="submitted" class="button1">
                Submit
              </v-btn>
              <v-card>
                <v-card-title class="headline">
                ...
                <v-card-text>
                  You cannot edit your application once you've submitted.
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn
                  ...
                  <v-btn color="green darken-1" flat @click="submitApplication">
                    Yes
                  </v-btn>
                </v-card-actions>
              </v-card>
            ...
            <v-dialog v-model="confirmClear" persistent max-width="400">
              <v-btn slot="activator" :disabled="submitted" class="button2">
                Clear
              </v-btn>
              <v-card>
                <v-card-title class="headline">
                  Are you sure you'd like to clear the form?
                </v-card-title>
                <v-card-text>
                  This will overwrite perviously saved data.
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn
                    color="red darken-1"
                    flat
                    @click.native="confirmClear = false"
                  >
                    No
                  </v-btn>
                  <v-btn color="green darken-1" flat @click="clearForm">
                    Clear
                  </v-btn>
                </v-card-actions>
              </v-card>
    ...
    <v-layout row justify-center>
      <v-dialog v-model="deadline" persistent max-width="450" min-height="350">
        <v-card>
          <v-card-title class="mx-auto gg headline">
            Sorry, DeltaHacks V applications are now closed!
          </v-card-title>
          <v-card-text>
