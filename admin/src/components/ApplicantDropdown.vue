    ...
    <v-layout row wrap>
      <v-flex xs12 md6 lg6>
        <v-card-title primary-title>
          <div>
            <h3
              class="headline mb-0"
            >{{ applicant.name + ' ' + applicant.lastname }} #{{ applicant.index }}</h3>
          </div>
        </v-card-title>
        <v-layout row wrap>
            ...
            <v-expansion-panel id="panel" popout>
              <v-expansion-panel-content v-model="topcard">
                ...
                <v-card>
                  <v-layout row wrap>
                    ...
                    <v-flex xs12 md6 lg6>
                      <v-card-text>{{ applicant.name + ' ' + applicant.lastname}}</v-card-text>
                      ...
                      <v-card-text>
                        <a
                          v-bind:href="applicant.github"
                          target="_blank"
                        >{{ applicant.github == '' ? '-' : applicant.github }}</a>
                      </v-card-text>
                      <v-card-text>
                        <a
                          v-bind:href="applicant.linkedin"
                          target="_blank"
                        >{{ applicant.linkedin == '' ? '-' : applicant.linkedin }}</a>
                      </v-card-text>
                      <v-card-text>
                        <a
                          v-bind:href="applicant.website"
                          target="_blank"
                        >{{ applicant.website == '' ? '-' : applicant.website }}</a>
                      </v-card-text>
                      <v-card-text>
                        <a
                          v-bind:href="applicant.devpost"
                          target="_blank"
                        >{{ applicant.devpost == '' ? '-' : applicant.devpost }}</a>
                      </v-card-text>
              ...
              <v-expansion-panel-content>
                ...
                <v-card>
                  <v-card-title
                    class="questionTitle"
                  >Tell us about a project you worked on/ thing you made/ internship you did/ course you took that you are really passionate about, and why?*</v-card-title>
                  <v-card-text>{{ applicant.q1 }}</v-card-text>
                  <v-card-title
                    class="questionTitle"
                  >Why do you want to come to Deltahacks V, and what is one thing that you are passionate to bring to this year's hackathon?*</v-card-title>
                  <v-card-text class="responseText">{{ applicant.q2 }}</v-card-text>
                  <v-card-title
                    class="questionTitle"
                  >If you could teleport to anywhere in the world right now, where would you go and why?*</v-card-title>
                  ...
                  <v-card-title
                    class="questionTitle"
                  >I give Microsoft permission to contact me, send me promotion material, and share resources to help me prepare for the hackathon.</v-card-title>
        ...
        <vue-slider
          :disabled="applicant.decision.reviewers.length >= 3 || isReviewed || !$store.state.currentUserIsAuthorizedReviewer"
          id="slider"
          v-model="score"
          :piecewise="false"
      ...
      <v-flex xs12 md6 lg6>
        <v-card color id="docCard">
          ...
          <iframe
            id="resume"
            v-if="applicant.documents[0] || applicant.documents"
            :src="applicant.documents[0] ? applicant.documents[0].download_link : (applicant.documents.download_link ? applicant.documents.download_link : 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf') "
          ></iframe>
