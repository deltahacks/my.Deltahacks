<template>
  <v-card>
    ...
    <div class="text-xs-center">
      <v-pagination v-model="page" :length="this.numApplicants" circle @input="nextPage"></v-pagination>
    </div>
  </v-card>
  ...
  methods: {
    isMarked(app) {
      ...
      app.decision.reviewers.forEach((val, ind) => {
        ...
        if (
          val.reviewer === this.$store.state.firebase.auth().currentUser.email
        ) {
          console.log("NIBBA");
          return true;
        }
  ...
  async mounted() {
    ...
    await db
      ...
      .then(snap => {
        ...
        console.log(this.numApplicants);
      });
